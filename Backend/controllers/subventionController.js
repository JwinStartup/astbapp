...
const SubventionOrganisation = require('../models/subventionOrganisation.js')
...
/*
   for(let i=0;i<req.body.apport.length;i++){ 
    ...
    SubventionOrganisation.findByIdAndUpdate(req.body._id,{$push:{apport:doc.id}}))
   
  }
     
       var eP = await  SubventionOrganisation.findByIdAndUpdate(req.body._id,{
...
const voir =async (req,res,next)=>{
  try {
    ...
    const p= await SubventionOrganisation.findById(req.params.id).populate('associe')
    res.json(p)
    console.log(p)
  } catch (error) {
    console.log(error)
