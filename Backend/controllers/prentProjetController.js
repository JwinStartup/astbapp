...
const modifie = async (req, res, next) => {
  try {
    ...
    if (req.body.autreplan) {
      for (let i = 0; i < req.body.autreplan.length; i++) {
        await new autrePlan({
          nom: req.body.autreplan[i].nom,
          description: req.body.autreplan[i].description,
        })
          .save()
          .then(
            async (doc) =>
              await Presentprojet.findByIdAndUpdate(req.body._id, {
                $push: { autreplan: doc.id },
              })
          );
      }
    }

    if (req.body.realisation) {
      for (let i = 0; i < req.body.realisation.length; i++) {
        await new realisation({
          datedebut: req.body.realisation[i].datedebut,
    ...
    if (req.body.associe) {
      for (let i = 0; i < req.body.associe.length; i++) {
        await new associe({ nom: req.body.associe[i].nom }).save().then(
          async (doc) =>
            await Presentprojet.findByIdAndUpdate(req.body._id, {
              $push: { associe: doc.id },
            })
...
const voir = async (req, res, next) => {
  try {
    ...
    const p = await Presentprojet.findById(req.params.id)
      .populate("autreplan")
      .populate("associe")
      .populate("realisation");
    res.json(p);
