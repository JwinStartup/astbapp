const pretRembourse = require('../models/pretRembourse.js')
const Pret = require('../models/pret.js')
...
/*
   for(let i=0;i<req.body.apport.length;i++){ 
    ...
    Pret.findByIdAndUpdate(req.body._id,{$push:{apport:doc.id}}))
   
  }
     
       var eP = await  Pret.findByIdAndUpdate(req.body._id,{
...
const voir =async (req,res,next)=>{
  try {
    ...
    const p= await Pret.findById(req.params.id).populate('associe')
    res.json(p)
    console.log(p)
  } catch (error) {
    console.log(error)
