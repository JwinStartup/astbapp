      ...
      <View style={styles.section}>
        ...
        <View style={styles.sousSection}>
          <Text style={{ fontSize: "16px", textAlign: "center", color: "red" }}>
            Recherche financement - Appel à des associés{" "}
          </Text>
        </View>

        <View style={styles.sousSection}>
          <Text>
            Quelles sont les personnes auxquelles vous comptez-vous associer
            pour céer votre entreprise ?:{data?.nombre_personne_contact}
          </Text>
        </View>

        <View style={styles.sousSection}>
          <Text style={{ margin: "3px", textAlign: "center" }}>
            (liste des personnes déjà contactées et qui sont engagés
            officiellement avec vous){" "}
          </Text>
          <View style={styles.sousSection}>
...
export default function AppelAssociepdf() {
  ...
  const datating = async () => {
    const reponses = await fetchWrapper.get(
      `https://astrainingbusiness-crud.onrender.com/api/appelAssocie/voir/${id}`
    );
    if (reponses) {
      setData(reponses);
